...
from chessbot.srv import ChessImage
...
image = []
engine = chess.engine.SimpleEngine.popen_uci("/home/chess-bot/Chess-Bot-Py/Realtime-OpenCV-Chess/Stockfish-master/src/stockfish") # stockfish engine
...
def show_game(game_img,board,player_move):
    ...
    if board.is_check() :
    	cv2.putText(game_img,"{}".format(board.is_check()),(1000,580),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
    else :
    ...
    if board.is_checkmate() :
    	cv2.putText(game_img,"{}".format(board.is_checkmate()),(1090,620),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
    else :
...
def decoder(data):
    ...
    jpg_original = base64.b64decode(data.immsg)
...
rospy.wait_for_service('Chess_bot')
ser_proxy = rospy.ServiceProxy('Chess_bot', ChessImage)

#data=ser_proxy()

#img_buffer=decoder(data)

#rospy.loginfo("recieving video frame")

#cv2.imshow("hehe",img_buffer)
    ...
    if answer == "y" or answer == "Y":
        ...
        while True:
                try:
                    ...
                    cv2.imshow("Setup Up Camera Position",img_buffer)
                    key = cv2.waitKey(1)
                    if key == ord('q'):
                        cv2.destroyAllWindows()
                        break
...
#view boxes
while True:
    print("Do you want to see Boxes on Chess board [y/n]:",end=" ")
    ans = str(input())
    if ans == 'y' or ans == "Y":
        # show boxes
        #ret , img = cv2.VideoCapture(-1).read()
        ...
        print("No Output boxes generated")
        break
    else:
        print("Enter valid input")
...
while 1:
    ...
    if board.turn == False and board.is_checkmate() == False:
        ...
        #ret , img_1 = cv2.VideoCapture(-1).read()
        i=0
        while(i<15):
          data = ser_proxy()
          i=i+1
        ...
        #ret , img_2 = cv2.VideoCapture(-1).read()
